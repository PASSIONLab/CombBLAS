...
extern double bu_local;
extern double bu_update;
extern double bu_rotate;
...
SpDCCols<int,bool>::SpColIter* CalcSubStarts(SpParMat<IT,bool,UDER> & A, FullyDistSpVec<IT,VT> & x, BitMapCarousel<IT,VT> &done) {
  ...
  MPI::Intracomm RowWorld = cg->GetRowWorld();
  RowWorld.Bcast(&rowuntil, 1, MPIType<IT>(), 0);
...
void UpdateParents(MPI::Intracomm &RowWorld, pair<IT,IT> *updates, int num_updates, FullyDistVec<IT,VT> &parents, int source, int dest, BitMapFringe<int64_t,int64_t> &bm_fringe) {
  int send_words = num_updates<<1, recv_words;
  RowWorld.Sendrecv(&send_words, 1, MPI::INT, dest, PUPSIZE,
                    &recv_words, 1, MPI::INT, source, PUPSIZE);
  pair<IT,IT>* recv_buff = new pair<IT,IT>[recv_words>>1];
  RowWorld.Sendrecv(updates, send_words, MPIType<IT>(), dest, PUPDATA,
                    recv_buff, recv_words, MPIType<IT>(), source, PUPDATA);
...
void BottomUpStep(SpParMat<IT,bool,UDER> & A, FullyDistSpVec<IT,VT> & x, BitMapFringe<int64_t,int64_t> &bm_fringe, FullyDistVec<IT,VT> & parents, BitMapCarousel<IT,VT> &done, SpDCCols<int,bool>::SpColIter* starts)
{
  ...
  World.Sendrecv(&my_coluntil, 1, MPIType<IT>(), diagneigh, TROST, &coluntil, 1, MPIType<IT>(), diagneigh, TROST);
  ColWorld.Bcast(&coluntil, 1, MPIType<IT>(), 0);
  RowWorld.Bcast(&rowuntil, 1, MPIType<IT>(), 0);
