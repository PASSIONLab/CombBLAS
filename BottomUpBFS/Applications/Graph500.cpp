...
int main(int argc, char* argv[])
{
	...
	}		
	{
		...
		if(string(argv[1]) == string("Input")) // input option
		{
			...
			ALocalT = PSpMat_s32p64(Aeff.seq().TransposeConst(), Aeff.getcommgrid());
      #ifdef THREADED
			ostringstream tinfo;
		...
		else if(string(argv[1]) == string("Binary"))
		{
			...
			// should make ALocalT, but hard to do without Aeff
		#ifdef THREADED	
			ostringstream tinfo;
			tinfo << "Threading activated with " << cblas_splits << " threads" << endl;
			SpParHelper::Print(tinfo.str());
		...
		#ifndef NOPERMUTE
			Aeff(nonisov, nonisov, true);	// in-place permute to save memory	
			SpParHelper::Print("Dropped isolated vertices from input\n");	
			Aeff.PrintInfo();
		#endif
		...
		for(int trials =0; trials < MAXTRIALS; trials++)	// try different algorithms for BFS if MAXTRIALS > 1
		{
			...
			MPI::COMM_WORLD.Gather(&bottomup_convert, 1, MPI::DOUBLE, bu_convert, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&bottomup_total, 1, MPI::DOUBLE, bu_total, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&bottomup_allgather, 1, MPI::DOUBLE, bu_ag_all, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&bottomup_sendrecv, 1, MPI::DOUBLE, bu_sr_all, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&cblas_allgathertime, 1, MPI::DOUBLE, td_ag_all, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&cblas_alltoalltime, 1, MPI::DOUBLE, td_a2a_all, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&cblas_transvectime, 1, MPI::DOUBLE, td_tv_all, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&cblas_mergeconttime, 1, MPI::DOUBLE, td_mc_all, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&cblas_localspmvtime, 1, MPI::DOUBLE, td_spmv_all, 1, MPI::DOUBLE, 0);
			MPI::COMM_WORLD.Gather(&cblas_ewisemulttime, 1, MPI::DOUBLE, td_ewm_all, 1, MPI::DOUBLE, 0);

			double bu_local_total = 0;
			double bu_update_total = 0;
			double bu_rotate_total = 0;

			MPI::COMM_WORLD.Allreduce(&bu_local, &bu_local_total, 1, MPI::DOUBLE, MPI::SUM);
			MPI::COMM_WORLD.Allreduce(&bu_update, &bu_update_total, 1, MPI::DOUBLE, MPI::SUM);
			MPI::COMM_WORLD.Allreduce(&bu_rotate, &bu_rotate_total, 1, MPI::DOUBLE, MPI::SUM);
