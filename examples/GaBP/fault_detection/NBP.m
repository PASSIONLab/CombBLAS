...
function [m,sig,mp]=meanvarmaxpdf(pdf, xx)
  pdf=pdf/sum(pdf);
   [mm,ind]=max(pdf);
   mp=xx(ind);
   loc_plus=find(xx>=0); % locations of positive and negative components
   loc_minus=find(xx<0);
   p_plus=sum(pdf(loc_plus)); % probabilities of having +1 and -1
   p_minus=sum(pdf(loc_minus));
   m=p_plus-p_minus; % we give (+1,-1) weights to positive/negative
