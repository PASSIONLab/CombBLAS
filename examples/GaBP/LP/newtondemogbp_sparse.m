...
for r=1:max_round;
   ...
   %lines, and add instead:  
   hx=inv(A'*(inv(D)^2)*A)*c;
   bigA = [eye(2)*1e-10 A'; A -D.^2];
   %bigA2 = [eye(2)*1e-10 A'; A -D.^2];
   bigb = [c', zeros(1,11) ]';
   %bigb2 = [ zeros(1,2) b'];
   %hx = bigA\bigb;
   [hx2,j] = GBP2(bigA, bigb, 100, 1e-5);
   norm(hx'-hx2(1:2))
   %saveGenaral('LP_small.txt', bigA, bigb2, bigA\bigb);
   %hx = hx(1:2);
...
%    x(:,r+1) = newx; 
    if (c'*hx/2 < NT_TOL)
       disp(['converged to tol ', num2str(-c'*hx/2) , ' in round', num2str(r)]);
       break;
    end
