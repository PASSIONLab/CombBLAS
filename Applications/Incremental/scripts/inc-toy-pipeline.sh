...
SYSTEM=panda
CORE_PER_NODE=8 # Never change. Specific to the system
PER_NODE_MEMORY=32 # Never change. Specific to the system
N_NODE=1
PROC_PER_NODE=1
N_PROC=$(( $N_NODE * $PROC_PER_NODE ))
THREAD_PER_PROC=$(( $CORE_PER_NODE / $PROC_PER_NODE ))
...
BINARY=$HOME/Codes/CombBLAS/_build/Applications/Incremental/inc-toy-pipeline
...
SELECTIVE_PRUNE_THRESHOLD=10 
#INPUT_DIR=$CFS/m1982/taufique/virus-lcc-incremental/"$N_SPLIT"-split/
INPUT_DIR=$CFS/$DATA_NAME-incremental/"$N_SPLIT"-split/
INFILE_PREFIX=geom-150
BASELINE=1

if [ "$BASELINE" == 1 ]; then
    OUTPUT_DIR_NAME=incremental-baseline.$DATA_NAME.$N_SPLIT.$INCREMENTAL_START.$SYSTEM.node_"$N_NODE".proc_"$PROC_PER_NODE".thread_"$THREAD_PER_PROC"
else
