...
void reversePermutation(SpParMat<IT, NT, DER>&M, FullyDistVec<IT, LBL>& lbl, FullyDistVec<IT, IT>& clust, FullyDistVec<IT, IT>& permMap){
    ...
    FullyDistSpVec<IT, IT> clustSp(clust); 
    FullyDistSpVec<IT, IT> origClustSp = SpMV<PTFF>(Q, clustSp, false);
    clust = FullyDistVec<IT, IT>(origClustSp);
}
    ...
    if(argc < 9)
    {
        ...
        for (int i = 1; i < argc; i++)
        {
            ...
            else if (strcmp(argv[i],"--summary-threshold")==0)
            {
                ...
                baseline = atoi(argv[i+1]);
                if(myrank == 0) printf("Run baseline incremental: %d\n", baseline);
            }
        }
        ...
        SpParMat<IT, NT, DER> M11all(fullWorld);
        ...
        for(int s = 1; s < nSplit; s++){
            ...
            FullyDistVec<IT, LBL> LOLblAll(fullWorld, totalLength, LBL{});
            FullyDistVec<IT, IT> permMap(fullWorld, totalLength, 0);
            FullyDistVec<IT, IT> permMapAll(fullWorld, totalLength, 0);
            FullyDistVec<IT, IT> isOld(fullWorld, totalLength, 0);
            FullyDistVec<IT, IT> isOldAll(fullWorld, totalLength, 0);
            
            incParam.shuffleVertexOrder = false; // Always shuffle vertices in incremental pipeline
            incParam.maxIter = 100; 
            {
                incParam.inflation = 1.2;
                incParam.dumpMarkovStates = true;
                incParam.markovStatePrefix = outputDir + std::to_string(s) ;
                incParam.prunelimit = 1.0/100.0;
                incParam.select = 10;
                incParam.recover_num = 14;
                incParam.recover_pct = .9; // we allow both 90 or .9 as input. Internally, we keep it 0.9
            }
            if (s < incStartStep){
            ...
            if(baseline){
                PrepIncMat(M11all, M12, M21, M22, L11, L22, Mall, LOLblAll, isOldAll, permMapAll, incParam); 
            }
            
            {
                if(myrank == 0) printf("Writing minc to file: %s\n", (outputDir + std::to_string(s) + std::string(".minc.mtx")).c_str());
                Minc.ParallelWriteMM(outputDir + std::to_string(s) + std::string(".minc.mtx"), base);
                FullyDistVec<IT, IT> LONum(fullWorld, totalLength, 0);
                convLblToNum(LOLbl, LONum);
                if(myrank == 0) printf("Writing minc labels to file: %s\n", (outputDir + std::to_string(s) + std::string(".minc.lbl")).c_str());
                LONum.ParallelWrite(outputDir + std::to_string(s) + std::string(".minc.lbl"), base);
            }
            ...
            if (s >= incStartStep){
                ...
                if (s == incStartStep -1){
                    if(baseline == 0) M11 = MSO; // Use the obtained summary as M11 for the next incremental step
                }
            }
            ...
            {
                if(myrank == 0) printf("Writing minc-summary to file: %s\n", (outputDir + std::to_string(s) + std::string(".minc-summary.mtx")).c_str());
                M11.ParallelWriteMM(outputDir + std::to_string(s) + std::string(".minc-summary.mtx"), base);
            }
