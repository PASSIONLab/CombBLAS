...
int main(int argc, char* argv[])
{
  ...
  if(argc < 7){
    ...
    t0 = MPI_Wtime();
    // 4 layers, column split
    SpParMat3D<int64_t, bool, SpDCCols < int64_t, bool >> ABool3D_cs = SpParMat3D<int64_t, bool, SpDCCols < int64_t, bool >>(ABool, 4, true, false);    // Non-special column split
    t1 = MPI_Wtime();
    if(myrank == 0) fprintf(stdout, "Time taken for converting A from 2D to 3D: %lf\n", t1-t0);
    ...
    while(true){
        ...
        t0 = MPI_Wtime();
        SpParMat3D<IT, NT, DER> Next3D_rs(Next, 4, false, false);
        t1 = MPI_Wtime();
        if(myrank == 0) fprintf(stdout, "Time taken for converting Next from 2D to 3D: %lf\n", t1-t0);

        t0 = MPI_Wtime();
        SpParMat3D<IT, NT, DER> Next3D_cs = Mult_AnXBn_SUMMA3D<SR, 
            double, SpDCCols<int64_t, double>, 
            int64_t, bool, double, SpDCCols<int64_t, bool>, SpDCCols<int64_t, double> >
            (ABool3D_cs, Next3D_rs);
        t1 = MPI_Wtime();
        if(myrank == 0) fprintf(stdout, "Time taken for Mult_AnXBn_SUMMA3D: %lf\n", t1-t0);

        t0 = MPI_Wtime();
        Next = Next3D_rs.Convert2D();
        t1 = MPI_Wtime();
        if(myrank == 0) fprintf(stdout, "Time taken for converting Next from 3D to 2D: %lf\n", t1-t0);
