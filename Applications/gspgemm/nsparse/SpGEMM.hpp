				 ...
				 long long int &flop)
{
    ...
    HIPCHECK(hipMalloc((void **)&(d_flop_per_row), sizeof(long long int) * (1 + a.nrow)));
  
    GS = div_round_up(a.nrow, BS);
    hipLaunchKernelGGL(set_flop_per_row, dim3(GS), dim3(BS), 0, 0,
					   a.d_rpt, a.d_colids, b.d_rpt, d_flop_per_row, a.nrow);
  
    long long int *tmp = (long long int *)malloc(sizeof(long long int) * a.nrow);
    hipMemcpy(tmp, d_flop_per_row, sizeof(long long int) * a.nrow, hipMemcpyDeviceToHost);
    flop = thrust::reduce(thrust::device, d_flop_per_row, d_flop_per_row + a.nrow);

    flop *= 2;
    hipFree(d_flop_per_row);
}
