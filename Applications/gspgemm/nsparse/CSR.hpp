...
public:
	...
	void memcpyHtD()
    {
        if (!device_malloc) {
            ...
            hipMalloc((void **)&d_rpt, sizeof(idType) * (nrow + 1));
            hipMalloc((void **)&d_colids, sizeof(idType) * nnz);
            hipMalloc((void **)&d_values, sizeof(valType) * nnz);
        }
        // cout << "Copying matrix data to GPU device" << endl;
        hipMemcpy(d_rpt, rpt, sizeof(idType) * (nrow + 1), hipMemcpyHostToDevice);
        hipMemcpy(d_colids, colids, sizeof(idType) * nnz, hipMemcpyHostToDevice);
        hipMemcpy(d_values, values, sizeof(valType) * nnz, hipMemcpyHostToDevice);
        device_malloc = true;
    }
    ...
    void memcpyDtH()
    {
        ...
        hipMemcpy(rpt, d_rpt, sizeof(idType) * (nrow + 1), hipMemcpyDeviceToHost);
        hipMemcpy(colids, d_colids, sizeof(idType) * nnz, hipMemcpyDeviceToHost);
        hipMemcpy(values, d_values, sizeof(valType) * nnz, hipMemcpyDeviceToHost);
    }
