/* pcnaup2.f -- translated by f2c (version 20050501).
...
/* FILE: naup2.F   SID: 1.6   DATE OF SID: 06/01/00   RELEASE: 1 */
...
/* Subroutine */ int pcnaup2_(integer *comm, integer *ido, char *bmat, 
	integer *n, char *which, integer *nev, integer *np, real *tol, 
	complex *resid, integer *mode, integer *iupd, integer *ishift, 
	integer *mxiter, complex *v, integer *ldv, complex *h__, integer *ldh,
	 complex *ritz, complex *bounds, complex *q, integer *ldq, complex *
	workl, integer *ipntr, complex *workd, real *rwork, integer *info, 
	ftnlen bmat_len, ftnlen which_len)
{
    ...
    /* Local variables */
    extern doublereal pscnorm2_(integer *, integer *, complex *, integer *);
    static integer i__, j;
    static real t0, t1, t2, t3;
    static integer kp[3], np0;
    static real cmpnorm_buf__;
    static integer nev0;
    static real eps23;
    ...
    static real rnorm;
    extern /* Subroutine */ int mpi_allreduce__(real *, complex *, integer *, 
    ...
    extern /* Subroutine */ int csortc_(char *, logical *, integer *, complex 
	    *, complex *, ftnlen), pcmout_(integer *, integer *, integer *, 
	    integer *, complex *, integer *, integer *, char *, ftnlen), 
	    pcvout_(integer *, integer *, integer *, complex *, integer *, 
	    char *, ftnlen), pivout_(integer *, integer *, integer *, integer 
	    *, integer *, char *, ftnlen), second_(real *), psvout_(integer *,
	     integer *, integer *, real *, integer *, char *, ftnlen), 
	    pcgetv0_(integer *, integer *, char *, integer *, logical *, 
	    integer *, integer *, complex *, integer *, complex *, real *, 
	    integer *, complex *, complex *, integer *, ftnlen), pcneigh_(
	    integer *, real *, integer *, complex *, integer *, complex *, 
	    complex *, complex *, integer *, complex *, real *, integer *);
    extern doublereal pslamch_(integer *, char *, ftnlen);
    extern /* Subroutine */ int pcnaitr_(integer *, integer *, char *, 
	    integer *, integer *, integer *, integer *, complex *, real *, 
	    complex *, integer *, complex *, integer *, integer *, complex *, 
	    complex *, integer *, ftnlen), pcngets_(integer *, integer *, 
	    char *, integer *, integer *, complex *, complex *, ftnlen), 
	    pcnapps_(integer *, integer *, integer *, integer *, complex *, 
...
/* L10: */
    ...
    if (getv0) {
	pcgetv0_(comm, ido, bmat, &c__1, &initv, n, &c__1, &v[v_offset], ldv, 
...
/*     %----------------------------------------------------------% */
    ...
    pcnaitr_(comm, ido, bmat, n, &c__0, nev, mode, &resid[1], &rnorm, &v[
...
L20:
    ...
    pcnaitr_(comm, ido, bmat, n, nev, np, mode, &resid[1], &rnorm, &v[
	    v_offset], ldv, &h__[h_offset], ldh, &ipntr[1], &workd[1], &workl[
	    1], info, (ftnlen)1);
	...
	*info = -9999;
    ...
    if (msglvl > 1) {
	psvout_(comm, &debug_1.logfil, &c__1, &rnorm, &debug_1.ndigit, "_nau"
...
/*        %--------------------------------------------------------% */
    ...
    pcneigh_(comm, &rnorm, &kplusp, &h__[h_offset], ldh, &ritz[1], &bounds[1],
	     &q[q_offset], ldq, &workl[1], &rwork[1], &ierr);
...
/*        %---------------------------------------------------% */
    ...
    pcngets_(comm, ishift, which, nev, np, &ritz[1], &bounds[1], (ftnlen)2);
...
/* L25: */
	...
	kp[1] = *np;
	kp[2] = nconv;
	pivout_(comm, &debug_1.logfil, &c__3, kp, &debug_1.ndigit, "_naup2: "
		"NEV, NP, NCONV are", (ftnlen)26);
	pcvout_(comm, &debug_1.logfil, &kplusp, &ritz[1], &debug_1.ndigit, 
		"_naup2: The eigenvalues of H", (ftnlen)28);
	pcvout_(comm, &debug_1.logfil, &kplusp, &bounds[1], &debug_1.ndigit, 
...
/* Computing 2nd power */
	    i__1 = kplusp;
	    pcvout_(comm, &debug_1.logfil, &kplusp, &workl[i__1 * i__1 + 1], &
		    debug_1.ndigit, "_naup2: Eigenvalues computed by _neigh:",
		     (ftnlen)39);
/* Computing 2nd power */
	    i__1 = kplusp;
	    pcvout_(comm, &debug_1.logfil, &kplusp, &workl[i__1 * i__1 + 
		    kplusp + 1], &debug_1.ndigit, "_naup2: Ritz eistmates co"
		    "mputed by _neigh:", (ftnlen)42);
	}
...
/*           %-----------------------------------------------% */
	...
	if (msglvl > 1) {
	    pcvout_(comm, &debug_1.logfil, &kplusp, &ritz[1], &debug_1.ndigit,
		     "_naup2: Sorted eigenvalues", (ftnlen)26);
	    pcvout_(comm, &debug_1.logfil, &kplusp, &bounds[1], &
		    debug_1.ndigit, "_naup2: Sorted ritz estimates.", (ftnlen)
		    30);
	}
...
/*           %---------------------------------------% */
	...
	if (nevbef < *nev) {
	    pcngets_(comm, ishift, which, nev, np, &ritz[1], &bounds[1], (
		    ftnlen)2);
	}
	...
	if (msglvl > 1) {
	    ...
	    pcvout_(comm, &debug_1.logfil, nev, &ritz[*np + 1], &
		    debug_1.ndigit, "_naup2: \"wanted\" Ritz values ", (
		    ftnlen)29);
	    pcvout_(comm, &debug_1.logfil, nev, &bounds[*np + 1], &
...
/*            %----------------------------------% */
	...
	pcvout_(comm, &debug_1.logfil, np, &ritz[1], &debug_1.ndigit, "_naup"
		"2: values of the shifts", (ftnlen)28);
	if (*ishift == 1) {
	    pcvout_(comm, &debug_1.logfil, np, &bounds[1], &debug_1.ndigit, 
...
/*        %---------------------------------------------------------% */
    ...
    pcnapps_(comm, n, nev, np, &ritz[1], &v[v_offset], ldv, &h__[h_offset], 
	    ldh, &resid[1], &q[q_offset], ldq, &workl[1], &workd[1]);
...
/*        %----------------------------------% */
	...
	cmpnorm_buf__ = q__1.r;
	mpi_allreduce__(&cmpnorm_buf__, &cmpnorm, &c__1, &
		mpipriv_1.mpi_complex__, &mpipriv_1.mpi_sum__, comm, &ierr);
	r__1 = cmpnorm.r;
	r__2 = r_imag(&cmpnorm);
	rnorm = sqrt(slapy2_(&r__1, &r__2));
    } else if (*(unsigned char *)bmat == 'I') {
	rnorm = pscnorm2_(comm, n, &resid[1], &c__1);
    }
    cnorm = FALSE_;
	...
	psvout_(comm, &debug_1.logfil, &c__1, &rnorm, &debug_1.ndigit, "_nau"
		...
		55);
	pcmout_(comm, &debug_1.logfil, nev, nev, &h__[h_offset], ldh, &
