/* pdgetv0.f -- translated by f2c (version 20050501).
...
/* Subroutine */ int pdgetv0_(integer *comm, integer *ido, char *bmat, 
	integer *itry, logical *initv, integer *n, integer *j, doublereal *v, 
	integer *ldv, doublereal *resid, doublereal *rnorm, integer *ipntr, 
	doublereal *workd, doublereal *workl, integer *ierr, ftnlen bmat_len)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int pdvout_(integer *, integer *, integer *, 
	    doublereal *, integer *, char *, ftnlen), second_(real *);
    extern doublereal pdnorm2_(integer *, integer *, doublereal *, integer *);
    extern /* Subroutine */ int pdlarnv_(integer *, integer *, integer *, 
	    integer *, doublereal *);
...
/*        %-----------------------------------------------------% */
	...
	if (! (*initv)) {
	    idist = 2;
	    pdlarnv_(comm, &idist, iseed, n, &resid[1]);
	}
...
L20:
	...
	mpi_allreduce__(&rnorm_buf__, &rnorm0, &c__1, &
		mpipriv_1.mpi_double_precision__, &mpipriv_1.mpi_sum__, comm, 
		ierr);
	rnorm0 = sqrt((abs(rnorm0)));
    } else if (*(unsigned char *)bmat == 'I') {
	rnorm0 = pdnorm2_(comm, n, &resid[1], &c__1);
    }
    *rnorm = rnorm0;
...
L30:
    ...
    mpi_allreduce__(&workl[*j + 1], &workl[1], &i__1, &
	    ...
	    ierr);
    i__1 = *j - 1;
    dgemv_("N", n, &i__1, &c_b29, &v[v_offset], ldv, &workl[1], &c__1, &c_b24,
...
L50:
    ...
    if (msglvl > 2) {
	pdvout_(comm, &debug_1.logfil, n, &resid[1], &debug_1.ndigit, "_getv"
