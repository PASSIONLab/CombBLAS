/* psseigt.f -- translated by f2c (version 20050501).
...
/* Subroutine */ int psseigt_(integer *comm, real *rnorm, integer *n, real *
	h__, integer *ldh, real *eig, real *bounds, real *workl, integer *
	ierr)
{
...
/*     %-------------------------------% */
    ...
    if (msglvl > 0) {
	psvout_(comm, &debug_1.logfil, n, &h__[(h_dim1 << 1) + 1], &
		debug_1.ndigit, "_seigt: main diagonal of matrix H", (ftnlen)
		33);
	if (*n > 1) {
	    i__1 = *n - 1;
    ...
    if (msglvl > 1) {
	psvout_(comm, &debug_1.logfil, n, &bounds[1], &debug_1.ndigit, "_sei"
		"gt: last row of the eigenvector matrix for H", (ftnlen)48);
    }
