/* psseupd.f -- translated by f2c (version 20050501).
...
/* \SCCS Information: */
/* FILE: seupd.F   SID: 1.10   DATE OF SID: 04/10/01 */

/* \EndLib */

/* ----------------------------------------------------------------------- */
/* Subroutine */ int psseupd_(integer *comm, logical *rvec, char *howmny, 
	logical *select, real *d__, real *z__, integer *ldz, real *sigma, 
	char *bmat, integer *n, char *which, integer *nev, real *tol, real *
	resid, integer *ncv, real *v, integer *ldv, integer *iparam, integer *
	ipntr, real *workd, real *workl, integer *lworkl, integer *info, 
	ftnlen howmny_len, ftnlen bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int slacpy_(char *, integer *, integer *, real *, 
	    ...
	    integer *, ftnlen), pivout_(integer *, integer *, integer *, 
    ...
    static integer leftptr;
    extern /* Subroutine */ int pssgets_(integer *, integer *, char *, 
	    integer *, integer *, real *, real *, real *, real *, ftnlen);
    static integer rghtptr;
...
/*     %---------------------------------------% */
	...
	bnorm2 = psnorm2_(comm, n, &workd[1], &c__1);
    }

    if (msglvl > 2) {
	psvout_(comm, &debug_1.logfil, ncv, &workl[irz], &debug_1.ndigit, 
		"_seupd: Ritz values passed in from _SAUPD.", (ftnlen)42);
	psvout_(comm, &debug_1.logfil, ncv, &workl[ibd], &debug_1.ndigit, 
		"_seupd: Ritz estimates passed in from _SAUPD.", (ftnlen)45);
    }
    if (*rvec) {
...
/*        %-------------------------------------% */
	...
	np = *ncv - *nev;
	ishift = 0;
	pssgets_(comm, &ishift, which, nev, &np, &workl[irz], &workl[bounds], 
		&workl[1], &workl[np + 1], (ftnlen)2);

	if (msglvl > 2) {
	    psvout_(comm, &debug_1.logfil, ncv, &workl[irz], &debug_1.ndigit, 
		    "_seupd: Ritz values after calling _SGETS.", (ftnlen)41);
...
/*        %-----------------------------------------------------------% */
	...
	if (msglvl > 1) {
	    ...
	    psvout_(comm, &debug_1.logfil, ncv, &workl[ihd], &debug_1.ndigit, 
		    "_seupd: NCV Ritz values of the final H matrix", (ftnlen)
		    45);
	    psvout_(comm, &debug_1.logfil, ncv, &workl[iw], &debug_1.ndigit, 
...
L30:
	...
	if (msglvl > 2) {
	    psvout_(comm, &debug_1.logfil, ncv, &workl[ihd], &debug_1.ndigit, 
		    "_seupd: The eigenvalues of H--reordered", (ftnlen)39);
	}
...
/* L100: */
	...
	psvout_(comm, &debug_1.logfil, &nconv, &d__[1], &debug_1.ndigit, 
		"_seupd: Untransformed converged Ritz values", (ftnlen)43);
	psvout_(comm, &debug_1.logfil, &nconv, &workl[ihb], &debug_1.ndigit, 
		"_seupd: Ritz estimates of the untransformed Ritz values", (
		ftnlen)55);
    } else if (msglvl > 1) {
	psvout_(comm, &debug_1.logfil, &nconv, &d__[1], &debug_1.ndigit, 
		"_seupd: Converged Ritz values", (ftnlen)29);
	psvout_(comm, &debug_1.logfil, &nconv, &workl[ihb], &debug_1.ndigit, 
		"_seupd: Associated Ritz estimates", (ftnlen)33);
    }
