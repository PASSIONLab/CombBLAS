/* pssaitr.f -- translated by f2c (version 20050501).
...
/* Subroutine */ int pssaitr_(integer *comm, integer *ido, char *bmat, 
	integer *n, integer *k, integer *np, integer *mode, real *resid, real 
	*rnorm, real *v, integer *ldv, real *h__, integer *ldh, integer *
	ipntr, real *workd, real *workl, integer *info, ftnlen bmat_len)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int slascl_(char *, integer *, integer *, real *, 
	    ...
	    psgetv0_(integer *, integer *, char *, integer *, logical *, 
...
/*        %--------------------------------% */
	...
	safmin = pslamch_(comm, "safmin", (ftnlen)6);
    }
...
L1000:
    ...
    if (msglvl > 2) {
	pivout_(comm, &debug_1.logfil, &c__1, &j, &debug_1.ndigit, "_saitr: "
		"generating Arnoldi vector no.", (ftnlen)37);
	psvout_(comm, &debug_1.logfil, &c__1, rnorm, &debug_1.ndigit, "_sait"
		"r: B-norm of the current residual =", (ftnlen)40);
    }
...
/*           %----------------------------------% */
	...
	rnorm_buf__ = sdot_(n, &resid[1], &c__1, &workd[ivj], &c__1);
...
/*        %--------------------------------------% */
    ...
    sgemv_("N", n, &j, &c_b59, &v[v_offset], ldv, &workl[1], &c__1, &c_b24, &
...
L80:
	...
	psvout_(comm, &debug_1.logfil, &c__2, xtemp, &debug_1.ndigit, "_nait"
		"r: re-orthonalization ; wnorm and rnorm are", (ftnlen)48);
    }
...
/*        %----------------------------------------------------% */
    ...
    sgemv_("T", n, &j, &c_b24, &v[v_offset], ldv, &workd[ipj], &c__1, &c_b51, 
...
/*        %----------------------------------------------------------% */
	...
	if (j < *k + *np) {
	    sscal_(n, &c_b59, &v[(j + 1) * v_dim1 + 1], &c__1);
	} else {
	    sscal_(n, &c_b59, &resid[1], &c__1);
	}
    }
...
/*        %------------------------------------% */
	...
	*ido = 99;
	...
	if (msglvl > 1) {
	    i__1 = *k + *np;
	    psvout_(comm, &debug_1.logfil, &i__1, &h__[(h_dim1 << 1) + 1], &
