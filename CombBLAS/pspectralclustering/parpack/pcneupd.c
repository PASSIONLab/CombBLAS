/* pcneupd.f -- translated by f2c (version 20050501).
...
/* ----------------------------------------------------------------------- */
/* Subroutine */ int pcneupd_(integer *comm, logical *rvec, char *howmny, 
	logical *select, complex *d__, complex *z__, integer *ldz, complex *
	sigma, complex *workev, char *bmat, integer *n, char *which, integer *
	nev, real *tol, complex *resid, integer *ncv, complex *v, integer *
	ldv, integer *iparam, integer *ipntr, complex *workd, complex *workl, 
	integer *lworkl, real *rwork, integer *info, ftnlen howmny_len, 
	ftnlen bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int pcmout_(integer *, integer *, integer *, 
	    ...
	    *, integer *, char *, ftnlen), clahqr_(logical *, logical *, 
	    integer *, integer *, integer *, complex *, integer *, complex *, 
	    integer *, integer *, complex *, integer *, integer *), cngets_(
	    integer *, integer *, char *, integer *, integer *, complex *, 
	    complex *, ftnlen), claset_(char *, integer *, integer *, complex 
	    *, complex *, complex *, integer *, ftnlen), ctrsen_(char *, char 
	    *, logical *, integer *, complex *, integer *, complex *, integer 
	    *, complex *, integer *, real *, real *, complex *, integer *, 
	    integer *, ftnlen, ftnlen), ctrevc_(char *, char *, logical *, 
...
/*     %------------------------------------% */
    ...
    if (msglvl > 2) {
	pcvout_(comm, &debug_1.logfil, ncv, &workl[irz], &debug_1.ndigit, 
		"_neupd: Ritz values passed in from _NAUPD.", (ftnlen)42);
	pcvout_(comm, &debug_1.logfil, ncv, &workl[ibd], &debug_1.ndigit, 
...
/*        %-------------------------------------% */
	...
	np = *ncv - *nev;
	ishift = 0;
	cngets_(comm, &ishift, which, nev, &np, &workl[irz], &workl[bounds], (
		ftnlen)2);

	if (msglvl > 2) {
	    pcvout_(comm, &debug_1.logfil, ncv, &workl[irz], &debug_1.ndigit, 
		    "_neupd: Ritz values after calling _NGETS.", (ftnlen)41);
	    pcvout_(comm, &debug_1.logfil, ncv, &workl[bounds], &
...
/*        %-------------------------------------------------------% */
	...
	if (msglvl > 1) {
	    pcvout_(comm, &debug_1.logfil, ncv, &workl[iheig], &
		    debug_1.ndigit, "_neupd: Eigenvalues of H", (ftnlen)24);
	    pcvout_(comm, &debug_1.logfil, ncv, &workl[ihbds], &
		    debug_1.ndigit, "_neupd: Last row of the Schur vector ma"
		    "trix", (ftnlen)43);
	    if (msglvl > 3) {
		pcmout_(comm, &debug_1.logfil, ncv, ncv, &workl[iuptri], &ldh,
		...
		*info = 1;
		...
		pcvout_(comm, &debug_1.logfil, ncv, &workl[iheig], &
			debug_1.ndigit, "_neupd: Eigenvalues of H--reordered",
			 (ftnlen)35);
		if (msglvl > 3) {
		    pcmout_(comm, &debug_1.logfil, ncv, ncv, &workl[iuptri], &
...
/* L40: */
		...
		ccopy_(&nconv, &workl[invsub + *ncv - 1], &ldq, &workl[ihbds],
			 &c__1);
		pcvout_(comm, &debug_1.logfil, &nconv, &workl[ihbds], &
			debug_1.ndigit, "_neupd: Last row of the eigenvector"
			" matrix for T", (ftnlen)48);
		if (msglvl > 3) {
		    pcmout_(comm, &debug_1.logfil, &nconv, ncv, &workl[invsub]
...
/* L60: */
	...
	pcvout_(comm, &debug_1.logfil, &nconv, &d__[1], &debug_1.ndigit, 
		"_neupd: Untransformed Ritz values.", (ftnlen)34);
	pcvout_(comm, &debug_1.logfil, &nconv, &workl[ihbds], &debug_1.ndigit,
