/* pcnaitr.f -- translated by f2c (version 20050501).
...
/* Subroutine */ int pcnaitr_(integer *comm, integer *ido, char *bmat, 
	integer *n, integer *k, integer *np, integer *nb, complex *resid, 
	real *rnorm, complex *v, integer *ldv, complex *h__, integer *ldh, 
	integer *ipntr, complex *workd, complex *workl, integer *info, ftnlen 
	bmat_len)
{
    /* Initialized data */
    ...
    static real rtemp[2], wnorm;
    extern /* Subroutine */ int mpi_allreduce__(complex *, complex *, integer 
    ...
    static real smlnum;
    extern /* Subroutine */ int pcvout_(), pcmout_(integer *, integer *, 
	    integer *, integer *, complex *, integer *, integer *, char *, 
	    ftnlen), pivout_(integer *, integer *, integer *, integer *, 
...
L1000:
	...
	pcvout_(comm, &debug_1.logfil, &c__1, rnorm, &debug_1.ndigit, "_nait"
...
/*           %--------------------------------------% */
    ...
    pcgetv0_(comm, ido, bmat, &itry, &c_false, n, &j, &v[v_offset], ldv, &
...
/*        %-------------------------------------% */
	...
	cnorm_buf__.r = q__1.r, cnorm_buf__.i = q__1.i;
	mpi_allreduce__(&cnorm_buf__, &cnorm, &c__1, &mpipriv_1.mpi_complex__,
		 &mpipriv_1.mpi_sum__, comm, &ierr);
	r__1 = cnorm.r;
	r__2 = r_imag(&cnorm);
	wnorm = sqrt(slapy2_(&r__1, &r__2));
    } else if (*(unsigned char *)bmat == 'I') {
	wnorm = pscnorm2_(comm, n, &resid[1], &c__1);
    }
...
L80:
	...
	rtemp[0] = wnorm;
	rtemp[1] = *rnorm;
	psvout_(comm, &debug_1.logfil, &c__2, rtemp, &debug_1.ndigit, "_nait"
		"r: re-orthogonalization; wnorm and rnorm are", (ftnlen)49);
	pcvout_(comm, &debug_1.logfil, &j, &h__[j * h_dim1 + 1], &
...
/*        %----------------------------------------------------% */
    ...
    mpi_allreduce__(&workl[j + 1], &workl[1], &j, &mpipriv_1.mpi_complex__, &
	    mpipriv_1.mpi_sum__, comm, &ierr);
...
/*        %---------------------------------------------% */
    ...
    q__1.r = -1.f, q__1.i = -0.f;
    cgemv_("N", n, &j, &q__1, &v[v_offset], ldv, &workl[1], &c__1, &c_b1, &
	    resid[1], &c__1, (ftnlen)1);
    caxpy_(&j, &c_b1, &workl[1], &c__1, &h__[j * h_dim1 + 1], &c__1);
...
/*        %-----------------------------------------------------% */
	...
	rnorm1 = pscnorm2_(comm, n, &resid[1], &c__1);
    }
	...
	if (msglvl > 2) {
	    rtemp[0] = *rnorm;
	    rtemp[1] = rnorm1;
	    psvout_(comm, &debug_1.logfil, &c__2, rtemp, &debug_1.ndigit, 
...
/* L110: */
	...
	if (msglvl > 2) {
	    i__1 = *k + *np;
	    i__2 = *k + *np;
	    pcmout_(comm, &debug_1.logfil, &i__1, &i__2, &h__[h_offset], ldh, 
