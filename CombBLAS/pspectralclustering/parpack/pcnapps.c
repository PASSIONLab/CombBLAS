/* pcnapps.f -- translated by f2c (version 20050501).
...
/* Subroutine */ int pcnapps_(integer *comm, integer *n, integer *kev, 
	integer *np, complex *shift, complex *v, integer *ldv, complex *h__, 
	integer *ldh, complex *resid, complex *q, integer *ldq, complex *
	workl, complex *workd)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int pcvout_(integer *, integer *, integer *, 
	    complex *, integer *, char *, ftnlen), claset_(char *, integer *, 
	    integer *, complex *, complex *, complex *, integer *, ftnlen), 
	    pcmout_(integer *, integer *, integer *, integer *, complex *, 
	    integer *, integer *, char *, ftnlen), second_(real *), pivout_(
...
/*     %----------------------------------------------% */
	...
	if (msglvl > 2) {
	    ...
	    pcvout_(comm, &debug_1.logfil, &c__1, &sigma, &debug_1.ndigit, 
...
/* Computing MAX */
		...
		if (msglvl > 0) {
		    ...
		    pcvout_(comm, &debug_1.logfil, &c__1, &h__[i__ + 1 + i__ *
...
/*     %-------------------------------------% */
    ...
    if (msglvl > 1) {
	pcvout_(comm, &debug_1.logfil, &c__1, &q[kplusp + *kev * q_dim1], &
		debug_1.ndigit, "_napps: sigmak = (e_{kev+p}^T*Q)*e_{kev}", (
		ftnlen)40);
	pcvout_(comm, &debug_1.logfil, &c__1, &h__[*kev + 1 + *kev * h_dim1], 
	...
	if (msglvl > 2) {
	    pcmout_(comm, &debug_1.logfil, kev, kev, &h__[h_offset], ldh, &
