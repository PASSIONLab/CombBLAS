/* pdseupd.f -- translated by f2c (version 20050501).
...
/* ----------------------------------------------------------------------- */
/* Subroutine */ int pdseupd_(integer *comm, logical *rvec, char *howmny, 
...
/*     %---------------------------------------% */
	...
	bnorm2 = pdnorm2_(comm, n, &workd[1], &c__1);
    }

    if (msglvl > 2) {
	pdvout_(comm, &debug_1.logfil, ncv, &workl[irz], &debug_1.ndigit, 
...
/*        %-------------------------------------% */
	...
	np = *ncv - *nev;
	ishift = 0;
	pdsgets_(comm, &ishift, which, nev, &np, &workl[irz], &workl[bounds], 
...
/*        %-----------------------------------------------------------% */
	...
	if (msglvl > 1) {
	    ...
	    pdvout_(comm, &debug_1.logfil, ncv, &workl[ihd], &debug_1.ndigit, 
		    "_seupd: NCV Ritz values of the final H matrix", (ftnlen)
		    45);
	    pdvout_(comm, &debug_1.logfil, ncv, &workl[iw], &debug_1.ndigit, 
...
/* L100: */
	...
	pdvout_(comm, &debug_1.logfil, &nconv, &d__[1], &debug_1.ndigit, 
		"_seupd: Untransformed converged Ritz values", (ftnlen)43);
	pdvout_(comm, &debug_1.logfil, &nconv, &workl[ihb], &debug_1.ndigit, 
