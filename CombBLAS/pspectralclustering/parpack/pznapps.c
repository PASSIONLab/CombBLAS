/* pznapps.f -- translated by f2c (version 20050501).
...
/* \SCCS Information: */
/* FILE: napps.F   SID: 1.3   DATE OF SID: 06/04/98 */
...
/* Subroutine */ int pznapps_(integer *comm, integer *n, integer *kev, 
	integer *np, doublecomplex *shift, doublecomplex *v, integer *ldv, 
	doublecomplex *h__, integer *ldh, doublecomplex *resid, doublecomplex 
	*q, integer *ldq, doublecomplex *workl, doublecomplex *workd)
{
    /* Initialized data */
...
/*     %--------------------------------------------% */
    ...
    zlaset_("All", &kplusp, &kplusp, &c_b8, &c_b5, &q[q_offset], ldq, (ftnlen)
	    3);
...
/*     %----------------------------------------------% */
	...
	if (msglvl > 2) {
	    ...
	    pzvout_(comm, &debug_1.logfil, &c__1, &sigma, &debug_1.ndigit, 
		    "_napps: Value of the shift ", (ftnlen)27);
	}
...
/* Computing MAX */
		...
		if (msglvl > 0) {
		    ...
		    pzvout_(comm, &debug_1.logfil, &c__1, &h__[i__ + 1 + i__ *
...
/*     %-------------------------------------------------% */
    ...
    if (h__[i__1].r > 0.) {
	zgemv_("N", n, &kplusp, &c_b5, &v[v_offset], ldv, &q[(*kev + 1) * 
		q_dim1 + 1], &c__1, &c_b8, &workd[*n + 1], &c__1, (ftnlen)1);
    }
...
/*     %----------------------------------------------------------% */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	i__2 = kplusp - i__ + 1;
	zgemv_("N", n, &i__2, &c_b5, &v[v_offset], ldv, &q[(*kev - i__ + 1) * 
		q_dim1 + 1], &c__1, &c_b8, &workd[1], &c__1, (ftnlen)1);
...
/*     %-------------------------------------% */
    ...
    if (msglvl > 1) {
	pzvout_(comm, &debug_1.logfil, &c__1, &q[kplusp + *kev * q_dim1], &
		debug_1.ndigit, "_napps: sigmak = (e_{kev+p}^T*Q)*e_{kev}", (
		ftnlen)40);
	pzvout_(comm, &debug_1.logfil, &c__1, &h__[*kev + 1 + *kev * h_dim1], 
	...
	if (msglvl > 2) {
	    pzmout_(comm, &debug_1.logfil, kev, kev, &h__[h_offset], ldh, &
