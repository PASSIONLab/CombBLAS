/* pscnorm2.f -- translated by f2c (version 20050501).
...
doublereal pscnorm2_(integer *comm, integer *n, complex *x, integer *inc)
{
    ...
    /* Local variables */
    static real buf, max__;
    static integer ierr;
    extern /* Subroutine */ int mpi_allreduce__(real *, real *, integer *, 
...
/*     %-----------------------% */
    ...
    /* Function Body */
    ret_val = scnrm2_(n, &x[1], inc);

    buf = ret_val;
    mpi_allreduce__(&buf, &max__, &c__1, &mpipriv_1.mpi_real__, &
	    mpipriv_1.mpi_max__, comm, &ierr);
    if (max__ == 0.f) {
	ret_val = 0.f;
    } else {
	d__1 = (doublereal) (ret_val / max__);
	buf = pow_dd(&d__1, &c_b3);
	mpi_allreduce__(&buf, &ret_val, &c__1, &mpipriv_1.mpi_real__, &
		mpipriv_1.mpi_sum__, comm, &ierr);
	ret_val = max__ * sqrt((dabs(ret_val)));
    }
