/* pdsaitr.f -- translated by f2c (version 20050501).
...
/* Subroutine */ int pdsaitr_(integer *comm, integer *ido, char *bmat, 
	integer *n, integer *k, integer *np, integer *mode, doublereal *resid,
	...
	integer *info, ftnlen bmat_len)
{
    ...
    extern /* Subroutine */ int pdvout_(integer *, integer *, integer *, 
	    ...
	    second_(real *), pdgetv0_(integer *, integer *, char *, integer *,
...
/*        %--------------------------------% */
	...
	safmin = pdlamch_(comm, "safmin", (ftnlen)6);
    }
...
L1000:
	...
	pdvout_(comm, &debug_1.logfil, &c__1, rnorm, &debug_1.ndigit, "_sait"
...
/*           %----------------------------------% */
	...
	rnorm_buf__ = ddot_(n, &resid[1], &c__1, &workd[ivj], &c__1);
...
/*        %--------------------------------------% */
    ...
    dgemv_("N", n, &j, &c_b59, &v[v_offset], ldv, &workl[1], &c__1, &c_b24, &
...
/*        %----------------------------------------------------% */
    ...
    dgemv_("T", n, &j, &c_b24, &v[v_offset], ldv, &workd[ipj], &c__1, &c_b51, 
...
/*        %----------------------------------------------------------% */
	...
	if (j < *k + *np) {
	    dscal_(n, &c_b59, &v[(j + 1) * v_dim1 + 1], &c__1);
	} else {
	    dscal_(n, &c_b59, &resid[1], &c__1);
	}
    }
...
/*        %------------------------------------% */
	...
	*ido = 99;
	...
	if (msglvl > 1) {
	    i__1 = *k + *np;
	    pdvout_(comm, &debug_1.logfil, &i__1, &h__[(h_dim1 << 1) + 1], &
