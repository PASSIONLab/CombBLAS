/* pssapps.f -- translated by f2c (version 20050501).
...
/* Subroutine */ int pssapps_(integer *comm, integer *n, integer *kev, 
	integer *np, real *shift, real *v, integer *ldv, real *h__, integer *
	ldh, real *resid, real *q, integer *ldq, real *workd)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int sscal_(integer *, real *, real *, integer *), 
	    ...
	    , real *, real *, integer *, real *, integer *), second_(real *);
    static real epsmch;
    ...
    extern /* Subroutine */ int slacpy_(char *, integer *, integer *, real *, 
	    ...
	    real *, real *, real *, integer *, ftnlen), pivout_(integer *, 
	    integer *, integer *, integer *, integer *, char *, ftnlen), 
	    psvout_(integer *, integer *, integer *, real *, integer *, char *
	    , ftnlen);
...
/*     %-----------------------% */
    ...
    if (first) {
	epsmch = pslamch_(comm, "Epsilon-Machine", (ftnlen)15);
	first = FALSE_;
    }
    itop = 1;
	...
	for (i__ = istart; i__ <= i__2; ++i__) {
		...
		if (msglvl > 0) {
		    ...
		    pivout_(comm, &debug_1.logfil, &c__1, &jj, &
			    ...
			    "mber.", (ftnlen)36);
		    psvout_(comm, &debug_1.logfil, &c__1, &h__[i__ + 1 + 
...
/*     %------------------------------------------% */
	...
	if (h__[i__ + 1 + h_dim1] <= epsmch * big) {
		...
		psvout_(comm, &debug_1.logfil, &c__1, &h__[i__ + 1 + h_dim1], 
...
/*     %-------------------------------------% */
	...
	psvout_(comm, &debug_1.logfil, &c__1, &q[kplusp + *kev * q_dim1], &
		...
		"or", (ftnlen)45);
	psvout_(comm, &debug_1.logfil, &c__1, &h__[*kev + 1 + h_dim1], &
		debug_1.ndigit, "_sapps: betak of the updated residual vector"
		, (ftnlen)44);
	psvout_(comm, &debug_1.logfil, kev, &h__[(h_dim1 << 1) + 1], &
