/* pzneupd.f -- translated by f2c (version 20050501).
...
static doublecomplex c_b37 = {1.,0.};
static logical c_true = TRUE_;
static doublecomplex c_b58 = {-1.,-0.};
...
/* \SCCS Information: */
/* FILE: neupd.F   SID: 1.6   DATE OF SID: 04/10/01 */

/* \EndLib */

/* ----------------------------------------------------------------------- */
/* Subroutine */ int pzneupd_(integer *comm, logical *rvec, char *howmny, 
	logical *select, doublecomplex *d__, doublecomplex *z__, integer *ldz,
	 doublecomplex *sigma, doublecomplex *workev, char *bmat, integer *n, 
	char *which, integer *nev, doublereal *tol, doublecomplex *resid, 
	integer *ncv, doublecomplex *v, integer *ldv, integer *iparam, 
	integer *ipntr, doublecomplex *workd, doublecomplex *workl, integer *
	lworkl, doublereal *rwork, integer *info, ftnlen howmny_len, ftnlen 
	bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int zlacpy_(char *, integer *, integer *, 
	    doublecomplex *, integer *, doublecomplex *, integer *, ftnlen), 
	    pzmout_(integer *, integer *, integer *, integer *, doublecomplex 
	    *, integer *, integer *, char *, ftnlen), pivout_(integer *, 
	    integer *, integer *, integer *, integer *, char *, ftnlen), 
	    zlahqr_(logical *, logical *, integer *, integer *, integer *, 
	    doublecomplex *, integer *, doublecomplex *, integer *, integer *,
	     doublecomplex *, integer *, integer *), zngets_(integer *, 
	    integer *, char *, integer *, integer *, doublecomplex *, 
	    doublecomplex *, ftnlen), zlaset_(char *, integer *, integer *, 
	    doublecomplex *, doublecomplex *, doublecomplex *, integer *, 
	    ftnlen), ztrsen_(char *, char *, logical *, integer *, 
	    doublecomplex *, integer *, doublecomplex *, integer *, 
	    ...
	    integer *), pzvout_(integer *, integer *, integer *, 
...
/*     %------------------------------------% */
    ...
    if (msglvl > 2) {
	pzvout_(comm, &debug_1.logfil, ncv, &workl[irz], &debug_1.ndigit, 
		"_neupd: Ritz values passed in from _NAUPD.", (ftnlen)42);
	pzvout_(comm, &debug_1.logfil, ncv, &workl[ibd], &debug_1.ndigit, 
...
/*        %-------------------------------------% */
	...
	np = *ncv - *nev;
	ishift = 0;
	zngets_(comm, &ishift, which, nev, &np, &workl[irz], &workl[bounds], (
		ftnlen)2);

	if (msglvl > 2) {
	    pzvout_(comm, &debug_1.logfil, ncv, &workl[irz], &debug_1.ndigit, 
		    "_neupd: Ritz values after calling _NGETS.", (ftnlen)41);
	    pzvout_(comm, &debug_1.logfil, ncv, &workl[bounds], &
...
/*        %-------------------------------------------------------% */
	...
	zlaset_("All", ncv, ncv, &c_b22, &c_b37, &workl[invsub], &ldq, (
		ftnlen)3);
	...
	if (msglvl > 1) {
	    pzvout_(comm, &debug_1.logfil, ncv, &workl[iheig], &
		    debug_1.ndigit, "_neupd: Eigenvalues of H", (ftnlen)24);
	    pzvout_(comm, &debug_1.logfil, ncv, &workl[ihbds], &
		    debug_1.ndigit, "_neupd: Last row of the Schur vector ma"
		    "trix", (ftnlen)43);
	    if (msglvl > 3) {
		pzmout_(comm, &debug_1.logfil, ncv, ncv, &workl[iuptri], &ldh,
			 &debug_1.ndigit, "_neupd: The upper triangular matr"
			"ix ", (ftnlen)36);
	    }
	}
	if (reord) {
...
/*           %-----------------------------------------------% */
		...
		*info = 1;
	    ...
	    if (msglvl > 2) {
		pzvout_(comm, &debug_1.logfil, ncv, &workl[iheig], &
			debug_1.ndigit, "_neupd: Eigenvalues of H--reordered",
			 (ftnlen)35);
		if (msglvl > 3) {
		    pzmout_(comm, &debug_1.logfil, ncv, ncv, &workl[iuptri], &
			    ldq, &debug_1.ndigit, "_neupd: Triangular matrix"
			    " after re-ordering", (ftnlen)43);
		}
	    }
	}
...
/*           %---------------------------------------------------% */
	    ...
	    if (workl[i__2].r < 0.) {
		zscal_(&nconv, &c_b58, &workl[iuptri + j - 1], &ldq);
		zscal_(&nconv, &c_b58, &workl[iuptri + (j - 1) * ldq], &c__1);
	    }
...
/* L40: */
		...
		zcopy_(&nconv, &workl[invsub + *ncv - 1], &ldq, &workl[ihbds],
			 &c__1);
		pzvout_(comm, &debug_1.logfil, &nconv, &workl[ihbds], &
			debug_1.ndigit, "_neupd: Last row of the eigenvector"
			" matrix for T", (ftnlen)48);
		if (msglvl > 3) {
		    pzmout_(comm, &debug_1.logfil, &nconv, ncv, &workl[invsub]
			    , &ldq, &debug_1.ndigit, "_neupd: The eigenvecto"
			    "r matrix for T", (ftnlen)36);
		}
	    }
...
/*           %----------------------------------------------% */
	    ...
	    ztrmm_("Right", "Upper", "No transpose", "Non-unit", n, &nconv, &
		    c_b37, &workl[invsub], &ldq, &z__[z_offset], ldz, (ftnlen)
...
/*     %-----------------------------------------------------------% */
	...
	for (k = 1; k <= i__1; ++k) {
	    i__2 = k;
	    z_div(&z__2, &c_b37, &workl[iheig + k - 1]);
...
/* L60: */
	...
	pzvout_(comm, &debug_1.logfil, &nconv, &d__[1], &debug_1.ndigit, 
		"_neupd: Untransformed Ritz values.", (ftnlen)34);
	pzvout_(comm, &debug_1.logfil, &nconv, &workl[ihbds], &debug_1.ndigit,
		 "_neupd: Ritz estimates of the untransformed Ritz values.", (
		ftnlen)56);
    } else if (msglvl > 1) {
	pzvout_(comm, &debug_1.logfil, &nconv, &d__[1], &debug_1.ndigit, 
		"_neupd: Converged Ritz values.", (ftnlen)30);
	pzvout_(comm, &debug_1.logfil, &nconv, &workl[ihbds], &debug_1.ndigit,
		 "_neupd: Associated Ritz estimates.", (ftnlen)34);
    }
...
/*        %---------------------------------------% */
	...
	zgeru_(n, &nconv, &c_b37, &resid[1], &c__1, &workev[1], &c__1, &z__[
		z_offset], ldz);
