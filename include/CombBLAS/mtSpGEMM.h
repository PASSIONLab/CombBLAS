...
#pragma omp parallel
   ...
   //std::tuple<IT,IT,NTO> * tuplesC = new std::tuple<IT,IT,NTO>[nnzc];
    ...
    std::vector<std::vector< std::pair<IT,IT>>> colindsVec(numThreads);
     ...
     std::vector<std::vector< std::pair<uint32_t,NTO>>> globalHashVecAll(numThreads); // uint32_t because integerSort accepts only uint32_t
...
#pragma omp parallel for
    ...
    //  hashSelected++;
            ...
            std::pair<uint32_t,NTO>* globalHashVec =  globalHashVecAll[myThread].data();
            // colinds.first vector keeps indices to A.cp, i.e. it dereferences "colnums" vector (above),
            // colinds.second vector keeps the end indices (i.e. it gives the index to the last valid element of A.cpnack)
            ...
            integerSort<NTO>(globalHashVecAll[myThread].data(), index);
            IT curptr = colptrC[i];
...
#pragma omp parallel for
    ...
    for(size_t i=0; i < Bdcsc->nzc; ++i)
    {
        ...
        //std::vector< std::pair<IT,NTO>> globalHashVec(ht_size);
        std::vector< std::pair<uint32_t,NTO>> globalHashVec(ht_size); //uint32_t because integerSort accepts only uint32_t
        ...
        //std::sort(globalHashVec.begin(), globalHashVec.begin() + index, sort_less<IT, NTO>);
        integerSort<NTO>(globalHashVec.data(), index);
                   ...
                   )
{
    ...
    for (size_t i = 0; i < Bcsc->n; ++i)
    {
        ...
        else                    // Hash Algorithm
        {
            ...
            std::vector< std::pair<uint32_t, NTO>> T(ht_size); // uint32_t because integerSort accepts only uint32_t
