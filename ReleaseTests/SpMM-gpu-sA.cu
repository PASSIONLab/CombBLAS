...
typedef int32_t TEST_IDX_T;
typedef float TEST_NNZ_T;
...
{	

	{
		...
		FullyDistDMat<TEST_IDX_T, TEST_NNZ_T> X(fullWorld, A.getncol(), k, 1.0);
		...
		{
			SpParHelper::Print(".");
			auto Y = SpMM_sA<PTSR>(A, X, stats);
			// Y.PrintToFile("SpMM-Y-sA-1.5D-P");
		}
