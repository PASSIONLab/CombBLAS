...
Takes a Python AST of a PcbUnaryPredicate-derived class and converts the __call__ method to
a semantic model corresponding to nodes from pcb_predicate_sm.
"""
...
class PcbUnaryPredicateFrontEnd(ast_tools.NodeTransformer):
    ...
    def visit_FunctionDef(self, node):
        ...
        return UnaryPredicate(self.visit(node.args), self.visit(node.body[0]))
    ...
    def visit_Return(self, node):
        # you can only return a boolean since this is a predicate
        return BoolReturn(self.visit(node.value))
