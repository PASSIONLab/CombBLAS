...
{
	A.ReadDistribute(filename, 0, false, Obj2ReadSaveHandler(), false, pario);
}

void pySpParMatObj2::save(const char* filename)
{
	A.SaveGathered(filename, Obj2ReadSaveHandler(), false);
}

// This load/save handler allows Python-defined callbacks.
class Obj2ReadSaveHandler_PythonCallback
{
public:
	...
	Obj2ReadSaveHandler_PythonCallback(PyObject* in_defaultConstructor, PyObject* in_loadCallback, PyObject* in_saveCallback):
		defaultConstructor(in_defaultConstructor), loadCallback(in_loadCallback), saveCallback(in_saveCallback)
	{
	}
	
public:
	Obj2 getNoNum(pySpParMatObj2::INDEXTYPE row, pySpParMatObj2::INDEXTYPE col) {
	
		// construct the object
		Obj2 ret;
		PyObject *tempSwigObj = SWIG_NewPointerObj(&ret, Obj2::SwigTypeInfo, 0);
...
pySpParMatObj2 pySpParMatObj2::TriU(int64_t k, bool inPlace)
{
	return pySpParMatObj2(A.PruneI(TriU_Prune_pred(k), inPlace));
}

pySpParMatObj2 pySpParMatObj2::TriL(int64_t k, bool inPlace)
{
	return pySpParMatObj2(A.PruneI(TriL_Prune_pred(k), inPlace));
}
